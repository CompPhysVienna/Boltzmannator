...
shown as a solid reference line in the transformation panel.  Toggling
...
\begin{itemize}
  \item \textbf{Left — control panel.}  A fixed-width panel with a header
        ...
        followed by an auto-rescale checkbox, a dark-mode switch, and
        Reset / Exit buttons.  The tabs are shown as a segmented control with
        icons; only the active tab's content scrolls, so the header image and
        the bottom controls stay fixed.  The \emph{Exit} button closes the
        current browser session (it does not stop the server for other users).
        A small set of selector dots next to the header image lets the user
        switch between the available portraits.
  \item \textbf{Right — figure.}  A Matplotlib figure split into two
        columns: the main four-panel display (left) and three histogram /
        loss panels (right).
\end{itemize}

\paragraph{Light and dark mode.}
The \emph{Dark} switch toggles between a light and a dark colour theme for the
whole interface, including the figure (backgrounds, axes, ticks, grid and
legend recolour accordingly); the default header portrait also changes with
the mode.  All plotted curves keep colours that remain legible in both themes.
...
(solid amber, when \emph{Show exact transformation} is ticked).
...
\begin{itemize}
  ...
  \item \textbf{Train!} — runs the optimiser in a background thread for
        ...
        while training, even when the loss oscillates near convergence.  The
        completion message also reports the average compute time per epoch.
